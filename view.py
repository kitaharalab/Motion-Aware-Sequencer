...
class View(tk.Frame):
    ...
    def createMusic(self):
        """楽曲の生成"""
        #盛り上がり度を求める
        self.excitement_array = self.model.chengeExcitement(self.pixcel_array)
        #状態を求める
        if self.selected_constitution_determine == 0:
            self.hmm_array = self.model.useHMM(self.excitement_array)
        else:
            self.hmm_array = self.model.useAutoHMM(self.excitement_array)
        if self.selected_fix_determine == 1:
            if self.selected_constitution_determine == 0:
                self.hmm_array,self.excitement_array = self.model.fixHmm(self.hmm_array,self.excitement_array)
            else:
                self.hmm_array,self.excitement_array = self.model.fixAutoHmm(self.hmm_array,self.excitement_array,self.model.section_array)
        #音素材を繋げる
        self.sound_list = self.model.choiceSound(self.excitement_array, self.hmm_array)
        #コードを付与する
        self.sound_list = self.model.giveChord(self.sound_list)
        #音素材を繋げる
        self.model.connectSound(self.sound_list)
        #描画を行う
